...
/**
 * Page for a {@link User} to change the password. Mounted on
 * /user/password/${id}
 */
@MountPath("/user/password/${id}")
public class UserPasswordPage extends DemoBasePage {

	private static final long serialVersionUID = 1L;

	static private Logger logger = Logger.getLogger(UserPasswordPage.class.getName());
	...
	public UserPasswordPage(IModel<User> model) {
	...
	public boolean canAccess(Optional<User> ouser) {
		...
		// a user can always change his own password
	...
	public void onInitialize() {
		...
		AjaxButtonToolbarItem<User> edit = new AjaxButtonToolbarItem<User>() {
			...
			protected void onCick(AjaxRequestTarget target) {
				UserPasswordPage.this.passwordEditor.onEdit(target);
			}
			...
			public boolean isVisible() {
				return passwordEditor.hasWritePermission();
			}
		};
	...
	protected Panel createHeaderPanel() {
		try {
			...
			bc.addElement(new BCElement(getLabel("password")));
